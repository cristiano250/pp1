class TV():
    ...
    def show_status(self):
        if (self.is_on==True) and len(self.channels)==0:
            print('TV jest włączony, kanał: ',self.channel_no, 'głośność: ',self.vol)
        elif (self.is_on==True) and len(self.channels)>0:
            print('TV jest włączony, kanał: ',self.channel_no,' czyli: ',self.channels[self.channel_no-1], 'głośność: ',self.vol)
        else:
